...
class DiscreteEnvironment(object):
    ...
    def __init__(self, resolution, lower_limits, upper_limits):
        ...
        for idx in range(self.dimension):
            self.num_cells[idx] = numpy.ceil((upper_limits[idx] - lower_limits[idx])/np.asarray(resolution[idx]))
    ...
    def ConfigurationToGridCoord(self, config): #DONE
        ...
        #
        coord = [0] * self.dimension
        r = np.asarray(self.resolution)
       	coord = numpy.floor((np.asarray(config) - np.asarray(self.lower_limits))/r)
       	return coord
    ...
    def GridCoordToConfiguration(self, coord): #Done
        ...
        ad=np.asarray(ad)
        r = np.asarray(self.resolution)
        config = numpy.multiply(numpy.add(coord,ad),r)  + np.asarray(self.lower_limits)
        return config
